...
def train(args):
    ...
    pre_noise = torch.randn(args.batch_size, 3, args.img_shape[0], args.img_shape[1]).cuda()
    pre_noise.requires_grad_(True)


    optimizer = Adam([pre_noise], lr=args.lr)
    ...
    for idx, data in enumerate(train_loader):
        ...
        for epoch in range(args.epoch):
            print('pre_noise', pre_noise.mean())
        ...
        _, adv_example = model(img, pre_noise)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_path', default=None, help='pretrained model state_dict path')
