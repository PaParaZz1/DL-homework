...
class MinusCrossEntropyLossTv(MinusCrossEntropyLoss):
    ...
    def forward(self, x, pred, gt):
        loss = super(MinusCrossEntropyLossTv, self).forward(pred, gt) + self.weight_tv*total_variation_loss(x)
        return loss
...
def train(args):
    ...
    train_set = datasets.CIFAR10(root=args.root, train=True, transform=transform)
    train_loader = DataLoader(train_set, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=False, pin_memory=True)
    # model
    ...
    if args.load_path is not None:
        ...
        print('load pretrained model in path: {}'.format(args.load_path), file=log_file)
    ...
    pre_noise = torch.zeros(args.batch_size, 3, args.img_shape[0], args.img_shape[1])
    ...
    for idx, data in enumerate(train_loader):
        ...
        for epoch in range(args.epoch):
            ...
            if args.loss_type == 'MinusCrossEntropy':
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_path', default='epoch_99_new.pth', help='pretrained model state_dict path')
